...
axios
  ...
  .then((response) => {
    ...
    renderDataPanel('card', getMoviesByPage(currentPage))  // render dataPanel
  })
  .catch(error => console.log(error))

// Render data function
function renderDataPanel(flag, data) {
  let rawHTML = ``
  if (flag === 'list') {
...
function searchMovies(data) {
  ...
  if (dataPanel.classList.contains('list-panel')) {
    renderDataPanel('list', getMoviesByPage(1))
  } else {
    renderDataPanel('card', getMoviesByPage(1))
  }
...
pageItem.addEventListener('click', function clickPage(event) {
  ...
  if (dataPanel.classList.contains('list-panel')) {
    renderDataPanel('list', getMoviesByPage(Number(currentPage)))
  } else {
    renderDataPanel('card', getMoviesByPage(Number(currentPage)))
  }
})
...
switchPanel.addEventListener('click', function clickSwitch(event) {
  ...
  if (target.classList.contains('list-btn')) {
    renderDataPanel('list', getMoviesByPage(currentPage))
  } else if (target.classList.contains('card-btn')) {
    renderDataPanel('card', getMoviesByPage(currentPage))
  }
})
