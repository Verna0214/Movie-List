...
axios
  ...
  .then((response) => {
    ...
    renderDataPanel(getMoviesByPage(currentPage))  // render dataPanel
  })
...
function renderDataPanel(data) {
  let rawHTML = ``
  if (dataPanel.dataset.mode === 'list') {
    dataPanel.removeAttribute('class')
    dataPanel.setAttribute('class', 'mt-5')
    ...
    dataPanel.removeAttribute('class')
    dataPanel.setAttribute('class', 'row row-cols-1 row-cols-md-4 g-4 mt-1')
    data.forEach((item) => {
      rawHTML += `
      <div class="col">
          <div class="card">
...
pageItem.addEventListener('click', function clickPage(event) {
  ...
  renderDataPanel(getMoviesByPage(Number(currentPage)))
